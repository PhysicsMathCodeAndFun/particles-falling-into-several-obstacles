...
for i in range((w//2 - 300)// size, (w//2 + 300) // size, 2):
    for j in range(5, (h - 1*size) // size, 2):
        p = Particles(i * size, j * size)
        p.side_walk = 1
        p.max_side_walk = 0
        p.color = [2,30,120]
        p.stop = True
...
def Update(screen):
    ...
    if t % 10 == 0 and len(particles) <= 800:
        particles.append(Particles((w//2), 0))
        
        
        
    for l in range(0, len(particles)):     
        
        if not particles[l].stop:
